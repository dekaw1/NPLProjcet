...
from tqdm import tqdm # 진행율 바 처리
import numpy as np
...
# >>> 훈련데이터 결측수량: 0
# >>> 테스트데이터 결측수량: 0
# *개발자 분석내용 - 최초에 훈련데이터에 5개의 결측데이터가 관측되었고 테스트데이터에 3개의 결측데이터가 관측되어 pandas 의 dropna 명령으로 제거하였다.

#seperate title[ 4. 중복데이터 확인 및 제거 ] ================
print(train_data["document"].count()) # 총 데이터의 수량
...
# >>> 49157
# >>> 중복된 데이터의 수: 840
# *개발자 분석내용 - 테스트 데이터 또한 중복 내용이 존재하고 있으나 훈련 대상 데이터가 아니므로 중복데이터 제거 하겠다.

#drop_duplicates()
train_data =  train_data.drop_duplicates(subset="document") #훈련데이터 중복 제거
test_data = test_data.drop_duplicates(subset="document") #테스트데이터 중복 제거
print("훈련 중복된 데이터의 수:",train_data["document"].count()-train_data["document"].nunique())
print("테스트 중복된 데이터의 수:",test_data["document"].count()-test_data["document"].nunique())
# >>> 훈련 중복된 데이터의 수: 0
# >>> 테스트 중복된 데이터의 수: 0
